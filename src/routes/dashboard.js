...
function createDashboardRouter({ pool, config }) {
  ...
  router.get('/dashboard/codes', async (req, res, next) => {
    try {
      ...
      if (['unused', 'used'].includes(req.query.status)) { values.push(req.query.status); where.push(`c.status=$${values.length}`); }
      ...
      const filters = `<form class="filters"><label>Category<select name="category"><option value="">All categories</option>${options}</select></label><label>Status<select name="status"><option value="">All statuses</option>${['unused','used'].map((status) => `<option value="${status}" ${req.query.status === status ? 'selected' : ''}>${status}</option>`).join('')}</select></label><button>Filter lifecycle</button><a class="button secondary" href="/dashboard/codes">Clear</a></form>`;
